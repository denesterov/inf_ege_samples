...
def victory(x, y):
    # return x + y >= 81 # 20907
    return x + y >= 207 # 27774

# STONES_MAX = 73 # 20907
STONES_MAX = 189 # 27774
...
def move(x, y, moves_left):
    ...
    if moves_left % 2 == 1:
        ...
        if any(victory(*st) for st in new_states):
            return False
        return all(move(*st, moves_left - 1) for st in new_states) # all or any



# 19 + replace `all` with `any` in line 26
for S in range(1, STONES_MAX):
    if move(BEGIN, S, 2) and not move(BEGIN, S, 1):
        print('19. S: ', S)

# 20:
for S in range(1, STONES_MAX):
    if move(BEGIN, S, 3) and not move(BEGIN, S, 1):
        print('20. S: ', S)

# 21:
for S in range(1, STONES_MAX):
    if move(BEGIN, S, 4) and not move(BEGIN, S, 2):
        print('21. S: ', S)
